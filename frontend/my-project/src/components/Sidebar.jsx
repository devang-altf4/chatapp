...
const Sidebar = () => {
  const { rooms, users, joinRoom, createRoom, createPrivateRoom, currentRoom } = useChat();
  const { logout, user } = useAuth();
  const [showNewRoom, setShowNewRoom] = useState(false);
  const [newRoomName, setNewRoomName] = useState('');
  const [activeTab, setActiveTab] = useState('rooms');
  const [creatingRoomStep, setCreatingRoomStep] = useState(1); // 1: name, 2: add users
    ...
    <div className="sidebar">
      <div className="sidebar-header">
        ...
        <div className="user-profile">
          ...
          <button className="btn-settings" title="Settings">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {creatingRoomStep === 1 ? (
                  ...
                  <div className="user-selection-step">
                    <div className="step-header">
                      <h4>Add Members to "{newRoomName}"</h4>
                      <p>{selectedUsers.length} selected • {filteredUsers.length} available</p>
                    </div>
                    <input
                      type="text"
                    ...
                    <div className="user-selection-list">
                      {users.length === 0 ? (
                        ...
                        <p className="empty-message">
                          {userSearchQuery ? 'No users match your search' : 'No other users available'}
                        </p>
                      ) : (
                        filteredUsers.map((otherUser) => {
                          const isSelected = selectedUsers.some(u => 
                            (u._id || u.id) === (otherUser._id || otherUser.id)
                          );
                          return (
                            <div
                            ...
                            >
                              ...
                              <div className="user-info-small">
                                ...
                                <div className="user-status-small">
                                  <span className={`status-dot-small ${otherUser.isOnline ? 'online' : ''}`}></span>
                    ...
                    <div className="step-actions">
                      ...
                      >
                        {selectedUsers.length > 0 ? `Create with ${selectedUsers.length} member${selectedUsers.length > 1 ? 's' : ''}` : 'Create Room'}
                      </button>
                      <button 
                        type="button" 
                        className="btn-back"
                        onClick={() => setCreatingRoomStep(1)}
                      >
                        Back
                      </button>
                      <button 
              ...
              rooms.map((room) => (
                ...
                >
                  ...
                  {currentRoom?._id === room._id && (
                    <div className="room-active-indicator"></div>
                  )}
                </div>
              ))
            )}
